...
public class Katana extends MeleeWeapon {
	...
	protected void duelistAbility(Hero hero, Integer target) {
		...
		int dmgBoost = augment.damageFactor( damageBoost(8 + Math.round(2f*buffedLvl()+2f*hero.weaponMastery)));
	...
	public String abilityInfo() {
		...
		int dmgBoost = damageBoost(8 + Math.round(2f*buffedLvl()+2f*hero.weaponMastery));
		if (levelKnown){
			...
			return Messages.get(this, "typical_ability_desc", min(0)+8, max(0)+8);
		}
	}
